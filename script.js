...
async function startBiometricScan() {
    ...
    try {
        // Simulate biometric scan with random success/failure for demo
        resultDiv.textContent = 'Scanning fingerprint...';
        ...
        if (isSuccess) {
            ...
            throw new Error('Fingerprint did not match');
        }
    } catch (error) {
        scannerCircle.classList.remove('scanning');
        scannerCircle.classList.add('error');
        // Immediately mark absent and return to dashboard
